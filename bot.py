...
API_ID = 28649550
API_HASH = "1dc2174831fb753d96568cf83f510b80"
PHONE = "+201285162937"
PASSWORD = "Wasd4949@"
BOT_TOKEN = "8441523610:AAHyaG2VmEJUK3Qc9SDcmbNU3P9yo7W6GQ8"
CHANNEL_IDENTIFIER = 2879978778
ADMIN_ID = 8010969668
...
if not CHANNEL_IDENTIFIER:
    log.critical("Missing TELEGRAM_CHANNEL environment variable. Set it to channel ID (-100...) or @username.")
    raise SystemExit("Set TELEGRAM_CHANNEL first (channel ID or @username).")
...
def save_state():
    try:
        with open(STATE_FILE, "w", encoding="utf-8") as f:
            json.dump({"users_welcomed": list(users_welcomed), "users_sent": list(users_final_replied)}, f, ensure_ascii=False, indent=2)
...
async def find_target_channel_once() -> Optional[object]:
    try:
        return await user_client.get_entity(CHANNEL_IDENTIFIER)
    except Exception as e:
        log.error(f"❌ Error finding target channel: {e}")
        return None
